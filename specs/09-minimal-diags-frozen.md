...
- Fresh vertical load (≥ 900 px, no usable stored height): diagnostics is header-only (Problems title + live count); list/empty-state nodes stay in the DOM but are clipped out of the client area (BR-901). Console is the flexible track (`minmax(80px, 1fr)`), ≥ 80 px; diagnostics is not an `fr` share.
...
- Clamps: content-derived header minimum ↔ lesser of 40 % right-column height or console 80 px floor. Viewport/layout-only clamps update memory/ARIA, not `localStorage`, until the visitor commits a resize.
...
- **Header-only default, body clipped not removed** — FR-038 count/list keep working without a second render mode (BR-901).
...
- **Bounds reuse FR-409** — 40 % right-column max, 80 px console floor; header min is content-derived.
